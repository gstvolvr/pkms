from typing import Optional, List, Dict, Any
import glob
import re
import requests
import os
import util
import collections

NICKNAMES = {
    ('Alicia', 'Andraca'): ['mama', 'mom'],
    ('Roberto', 'Oliver'): ['dad', 'papa'],
    ('Javier', 'Ramos'): ['javi'],
    ('Monica', 'Oliver'): ['moni'],
    ('Courtney', 'Oliver'): ['c']
}

def find_all_people() -> dict[Any, Any]:
    people_files = []
    # don't care about 'authors' etc
    for subdir in ['family', 'friends', 'co-workers', 'pets']:
        people_files.extend(util.find_all_files(f'{util.BASE_PATH}/people/{subdir}'))
    names_to_file = {tuple(file_name.split('/')[-1].replace('.md', '').split(' ')): file_name for file_name in people_files}
    # print(names_to_file)
    first_name_to_file = {}
    d = collections.defaultdict(int)
    for name in names_to_file:
        fn = name[0]
        d[fn] += 1

    # print('Duplicate first names:')
    for name, path in names_to_file.items():
        fn = name[0]
        #if d[fn] > 1:
        #    print(name)
        # if fn.lower() not in ('kyle', 'paige', 'arman', 'neel', 'marcos', 'carlos', 'shikha', 'nic', 'mona', 'courtney'):
        #     continue

        if (d[fn] == 1) and fn not in ('Juan', 'Jim', 'Tim', 'Ali', 'Alex', 'Will'):
            first_name_to_file[fn] = path

        for nickname in NICKNAMES.get(name, []):
            first_name_to_file[nickname] = path

    return first_name_to_file

def find_and_replace_name_with_link(text: str, file_path: str, names_to_path: dict):
    for name, path in names_to_path.items():
        pattern = util.get_name_pattern(name)
        path = os.path.relpath(path, start=file_path).replace(' ', '%20')
        if name.lower() in text.lower() and re.findall(pattern, text):
            pattern = util.get_name_pattern(name, to_replace=True)
            new_text = re.sub(pattern, f'[{name.t:itle()}]({path})', text)

            #if text != new_text:
            #    print('-'*100)
            #    print('before', name)
            #    print(path)
            #    print(text)
            #    print('-'*100)
            #    print('after', name)
            #    print(new_text)
            #    print('\n'*2)
            return new_text
    return None


def main():
    existing_files = glob.glob(f'{util.PENSIEVE_PATH}/*/*/*')
    names_to_path = find_all_people()
    print(names_to_path)
    # TODO: get this working again
    return
    n_files = 0
    for file_path in existing_files:
        with open(file_path, 'r') as r:
            text = r.read()
            if not text:
                print(f'{file_path} is empty')
                continue

            hyperlinked_text = find_and_replace_name_with_link(text, file_path, names_to_path)

            if not hyperlinked_text:
                continue

            n_files += 1

            print(hyperlinked_text)
            print(file_path)

        with open(file_path, 'w') as w:
            w.write(hyperlinked_text)
        #break
    print(f'number of files updated: {n_files}')


if __name__ == '__main__':
    main()
